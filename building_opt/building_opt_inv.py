...
def run_simulation():
    ...
    Tmix = (273.15+22)*np.ones(len(time)) + np.sin(2*3.14*(1/24.0/3600)*time)
    ...
    fmu_model = compile_fmu('ElectricNetwork.RCBuildingModel2_Inv', path)
    ...
    #  Tmix
    #  Price
    u = np.transpose(np.vstack((time, ihg, Tamb, Tgnd, sRadE, sRadN, sRadS, sRadW, Tmix, price)))
    
    sim_opts = model.simulate_options()
    sim_opts.update({'initialize':True,
                     'ncp':20*len(time)-1,
                    'CVode_options':{
                        'atol': 1e-7,
                        'discr': 'BDF',
    ...
    # Simulate
    res = model.simulate(input=(['u[1]', 'u[2]', 'u[3]', 'u[4]', 'u[5]', 'u[6]', 'u[7]', 'Tmix', 'price'], u), start_time = time[0], final_time = 3600*24, options=sim_opts)
